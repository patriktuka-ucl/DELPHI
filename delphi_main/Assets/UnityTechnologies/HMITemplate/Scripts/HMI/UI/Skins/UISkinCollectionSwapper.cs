        ...
        private static void ApplySkinToScene(GameObject target, UISkinCollectionData lastSkin, UISkinCollectionData newSkin)
        {
			...
			foreach (var skinnable in skinnables)
			{
				...
				*/
                ...
                {
                    // Print a warning if a named element is missing
                    Debug.LogWarning($"[UISkinCollectionSwapper] No matching skin data named '{skinData.name}' found in the new skin '{newSkin.name}' for skinnable '{skinnable.name}'. Ensure all skins have corresponding named elements.", skinnable);
                }
            }
        }
